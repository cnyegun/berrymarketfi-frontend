      ...
      <section className="mx-auto max-w-[1080px] px-6">
        <div className="grid gap-6 lg:grid-cols-3">
          {PLANS.map((plan) => (
            ...
            >
              {plan.highlighted && (
                <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-brand px-4 py-1 text-[14px] font-bold text-white">
                  Most popular
                </span>
              )}
              <h3 className="text-[21px] font-bold text-ink">{plan.name}</h3>
              <div className="mt-4 flex items-end gap-1.5">
                <span className="text-[46px] font-extrabold leading-none tracking-[-0.02em] text-ink">
                  {plan.price}
                </span>
                <span className="pb-1 text-[16px] text-muted">{plan.cadence}</span>
              </div>
              <p className="mt-3 text-[17px] leading-relaxed text-muted">{plan.desc}</p>

              <ul className="mt-6 flex-1 space-y-3">
                {plan.features.map((f) => (
                  <li key={f} className="flex items-start gap-2.5 text-[17px] text-ink/85">
              ...
              {plan.note && (
                <p className="mt-5 rounded-lg bg-brand-soft px-3 py-2 text-center text-[15px] font-semibold text-brand-dark">
                  {plan.note}
                </p>
              )}
              ...
              <a
                href="#"
                className={`mt-6 block rounded-xl px-6 py-3.5 text-center text-[17px] font-semibold transition-all hover:-translate-y-0.5 ${
                  plan.highlighted
        ...
        <div className="mt-10 divide-y divide-line border-y border-line">
          {FAQS.map((item) => (
            <div key={item.q} className="py-6">
              <h3 className="text-[20px] font-bold text-ink">{item.q}</h3>
              <p className="mt-2 text-[18px] leading-relaxed text-muted">{item.a}</p>
            </div>
          ))}
        </div>
      </section>
